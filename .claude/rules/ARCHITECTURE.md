...
- `feature/<имя>` — новая разработка; `fix/<имя>` — исправления/чистка.
- **Коммит-сообщение строго равно имени ветки** (без описаний, тел, эмодзи, AI-меток). Переопределяет глобальный формат `{номер} - {Название}`.
