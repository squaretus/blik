...
## Changes
- Added: `blik claude-statusline` — one ANSI line for Claude Code statusLine: avg
  P-core/E-core/GPU temps, RAM used, VRAM used, each with a ▁▂▃▄▅▆▇█ sparkline over
  30 min of history (`queryHistorySync`, 20 points). Color thresholds: temps
...
## How to test
- [ ] Unit: `StatuslineRendererTests` (sparkline scaling, thresholds, degradation), `MCPToolsTests` (dispatch, clamp, preset validation, daemon-down errors) — mock via `MCPMetricsSource`.
