...
- `ChartsVM.restartLiveHistory()` — (re)starts the ~4 s loop that pulls daemon history for the current live window into `liveHistory[metric]`. Runs for ANY live-window width (guard only `runtime.helperSupportsHistory` + `runtime.xpcClient != nil`) — the old `liveWindowSeconds > liveBufferSpan` gate that starved narrow (5/15 min) windows is gone. `liveBufferSpan` property removed. See bugs/live-narrow-window-empty-charts.md.
- `ChartsVM.publishLiveHistory(_:bucketSeconds:)` — the **single** publication point for live history. Defers the payload into `pendingLiveHistory` (`@ObservationIgnored`) while `scrolling`, skips writes when the payload equals the current `liveHistory`/`rangeBucketSeconds`. `setScrolling(false)` flushes the pending payload. `fetchLiveHistory` must route through it, never assign `liveHistory`/`rangeBucketSeconds` directly. See bugs/charts-scroll-freeze-observable-bypass.md.
- `ChartsVM.setScrolling(_ value: Bool)` — pauses render ticks during scroll; on scroll end flushes `pendingLiveHistory` then bumps `summaryTick`/`chartTick` once.
- `ChartsVM.liveMergeSplit(history:bufferSegments:) -> (history:tail:)` — `public static`, pure. Splits live-window data into "past" (daemon history truncated to `< tail.first.ts`) and "live tail" (the last contiguous buffer segment). Stale buffer fragments from earlier page visits are discarded — their region is covered by history. `ChartData.segments` (BlikApp) consumes it: buffer-only render only when history is absent, else merge history + tail.
...
- Live-chart data is always daemon-history-backed when the helper supports history: `restartLiveHistory` no longer depends on window width, so the `LiveMetricBuffer` (900 pts, fills only while the page is visible) is only a smooth-tail source, never the sole source. Any width-based gate reintroduces empty narrow windows.
- **Every write to `ChartsVM.liveHistory`/`rangeBucketSeconds` goes through `publishLiveHistory` and respects the scroll gate.** Heavy `MetricChart` bodies (BlikApp) read `liveHistory` via `ChartData.segments`; publishing past the gate rebuilds all visible charts mid-scroll (~4 s cadence) → scroll freeze. General rule for this `@Observable` VM layer: gate ALL observable writes that heavy bodies read, not only the explicit `chartTick`/`summaryTick` render ticks. `@Observable` does not deduplicate assignments — identical payloads must be diffed out by hand (`if dict != liveHistory`). See bugs/charts-scroll-freeze-observable-bypass.md.
- `liveMergeSplit` keeps the **last contiguous** buffer segment as the live tail and drops all earlier (stale) segments; history fills everything before the tail start. Assumes buffer segments are time-ordered and the last one is the current capture.
